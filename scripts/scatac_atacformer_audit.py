...
(scatac_atacformer_emb.npz). Mirrors scatac_audit.py exactly (LAC conformal,
...
d = np.load(os.path.join(OUT, "scatac_atacformer_emb.npz"), allow_pickle=True)
...
r=dict(model="Atacformer (databio, scATAC FM, zero-shot, self-written TransformerEncoder)",
  dataset="GSE174367 snATAC (subsample %d cells, full-peak->890k-region universe overlap)"%len(y), n_cells=int(len(y)),
...
json.dump(r, open(os.path.join(OUT,"scatac_atacformer_audit_result.json"),"w"), indent=2)
