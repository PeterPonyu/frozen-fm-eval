#!/usr/bin/env python3
"""Normalize heterogeneous external benchmark tables into a pooled long table.

Input tables live under ``--raw-pulls``. Outputs are ``pooled_long.csv``, an
optional Parquet copy, and ``sources_provenance.csv`` under ``--results-dir``.
This table-normalization pipeline has no stochastic step; ``--seed`` is
accepted for a consistent archive CLI and does not affect the result.
"""
...
def parse_args():
    ...
    parser.add_argument("--data-root", default=None, help="Alias for --raw-pulls when that flag is omitted")
    parser.add_argument("--raw-pulls", default=None, help="Directory containing the seven external source pulls")
    parser.add_argument("--results-dir", default=".", help="Directory for pooled output tables")
    parser.add_argument("--seed", type=int, default=20260623, help="Accepted for CLI consistency; normalization is deterministic")
    return parser.parse_args()


_ARGS = parse_args()
ROOT = os.path.abspath(_ARGS.results_dir)
RP = os.path.abspath(_ARGS.raw_pulls or _ARGS.data_root or "raw_pulls")
OUT = ROOT
