...
options(tikzMetricsDictionary = ".tikz_metrics")  # relative to cwd; script is always run from paper/
base <- Sys.getenv("SCFM_BASE", "..")  # repo subdir root; run from paper/ or set SCFM_BASE
figs <- file.path(base, "paper", "figs"); dir.create(figs, showWarnings = FALSE, recursive = TRUE)
...
f3 <- ggplot(d3, aes(regime, cov, fill = kind)) + geom_col(width = .65) +
  ...
  labs(x = NULL, y = "conformal coverage (target 0.90)", title = "conformal coverage") +
...
p4e <- ggplot(dm, aes(sub, ECE, fill=kind)) + geom_col(width=.7) +
  ...
  labs(x=NULL, y="ECE (lower = better)", title="calibration error") +
  theme(axis.text.x=element_text(angle=35, hjust=1, size=7))
...
f5 <- ggplot(v1[!is.na(v1$auroc),], aes(method, auroc, fill = kind)) + geom_col(width = .7) +
  ...
  labs(x = NULL, y = "cross-batch (LOBO) macro-AUROC", title = "cross-batch LOBO") +
...
f7 <- ggplot(rows, aes(method, cov_gap)) + geom_hline(yintercept=0, linetype="dashed", color="grey50") +
  ...
  labs(x=NULL, y="cross-batch conformal coverage gap", title="Simple baselines, 24 leakage-controlled atlases") +
...
p10b <- ggplot(sb, aes(compo, expr, color=kind)) + geom_point(size=2.2, alpha=.85) +
  ...
  labs(x="$R^2_{\\mathrm{compo}}$ (neighborhood composition)", y="$R^2_{\\mathrm{expr}}$ (single-cell expression)", title="niches are composition-defined", tag="B") +
  theme(legend.position="top", plot.tag.position=c(-0.02,1.06), plot.tag.location="panel", plot.tag=element_text(face="bold", size=11))
...
pL1 <- ggplot(v5b, aes(stratum, mse, fill=method)) + geom_col(position="dodge", width=.72) +
  ...
  labs(x=NULL, y="full-gene MSE (lower = better)", title="full-gene MSE") + theme(legend.position="top")
...
pR1 <- ggplot(ml, aes(method, lift)) + geom_hline(yintercept=0, linetype="dashed", color="grey50") +
  ...
  labs(x=NULL, y="accuracy@80\\% coverage $-$ full", title="abstention recovers accuracy") +
  theme(axis.text.x=element_text(angle=20, hjust=1))
...
pR2 <- ggplot(et) + geom_segment(aes(x=raw, xend=temp, y=reorder(atlas,raw), yend=reorder(atlas,raw)), color="grey60", arrow=arrow(length=unit(3,"pt"))) +
  ...
  labs(x="cross-batch ECE: raw $\\to$ temp-scaled", y="each row $=$ 1 of 24 atlases (top $=$ worst raw ECE)", title="temperature scaling reduces ECE") +
...
pKa <- ggplot(dcap, aes(cap, ece, fill=fm)) + geom_col(position="dodge", width=.7) +
  ...
  labs(x="readout capacity", y="cross-batch ECE (lower = better)", title="MLP-256 readout worsens ECE on both FMs") +
...
f13 <- ggplot(li, aes(iLISI, cLISI, color=method, shape=atlas)) +
  ...
  labs(x="iLISI (batch mixing) $\\to$ higher better", y="cLISI (cell-type purity) $\\to$ lower better",
...
p16a <- mk16("AUROC","niche $k$NN-AUROC ($\\rho_{F1}{=}0.98$)")
p16b <- mk16("F1","niche macro-F1 (peaks at $k{\\approx}100$)")
...
f17 <- ggplot(dz, aes(genes, auroc, color=atlas)) +
  ...
  annotate("text", x=2048, y=0.53, label="FM gene budget (2048)", hjust=-0.03, size=2.6, color="grey50", angle=90) +
