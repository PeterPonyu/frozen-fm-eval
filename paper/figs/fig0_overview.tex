...
\newcommand\scaleicon{\begin{tikzpicture}[baseline=(current bounding box.center)]
  ...
  \draw[cEval,line width=0.6pt] (-5pt,6pt)--(-5pt,4pt);
  \draw[cEval,line width=0.6pt] (-5pt,4pt)--(-6.5pt,1.5pt)--(-3.5pt,1.5pt)--cycle;
  \draw[cEval,line width=0.6pt] (5pt,6pt)--(5pt,4pt);
  \draw[cEval,line width=0.6pt] (5pt,4pt)--(3.5pt,1.5pt)--(6.5pt,1.5pt)--cycle;
...
\node[lane=cFM,minimum width=2.6cm,minimum height=0.7cm] (fm) at (3.4,1.5) {\transformer};
\node[stg=cFM,above=1pt of fm] {FOUNDATION MODEL};
\node[lane=cBase,minimum width=2.6cm,minimum height=0.7cm] (bl) at (3.4,-0.4)
...
\node[draw=black!30,rounded corners=2pt,minimum size=0.7cm] (fu) at (6.1,1.5) {\umap{0.9}};
\node[draw=black!30,rounded corners=2pt,minimum size=0.7cm] (bu) at (6.1,-0.4) {\umap{0.9}};
\node[sub] at (6.1,0.55) {cell\\embeddings};
% --- Stage 4: fair evaluation -----------------------------------------------
\node (sc) at (8.55,0.55) {\scaleicon};
...
\node (g1) at (10.9,0.72) {\miniparity};
...
% panel label. NOTE: this xshift is a no-op for A's ABSOLUTE page x. Because A is the
% leftmost element of this picture, it defines the bbox min-x, which the \makebox in
% main.tex then flushes to a fixed page position; moving the xshift only slides A
% relative to content that is re-flushed. A/B column alignment is therefore driven from
% panel B's xshift in fig0b_roadmap.tex (B shifted right to meet A), NOT from here.
\node[anchor=south west,font=\normalsize\bfseries] at ([xshift=-22pt,yshift=2pt]current bounding box.north west) {A};
\end{tikzpicture}}
